...
#'
#' The data-uninformed Parametric Bootstrap Cross-fitting Method (PBCM) generates synthetic data from two models of a phenomenon with given model parameter values, and then cross-fits the models to these data. The result is two distributions of the goodness of fit metric DeltaGoF = GoF1 - GoF2, where GoF1 is the fit of model 1 and GoF2 the fit of model 2.
#'
#' Functions \code{fun1} and \code{fun2} must take an argument named \code{data} in addition to any arguments specified in \code{args1} and \code{args2}; this is used to pass the synthetic data generated by \code{genfun1} and \code{genfun2}. \code{fun1} and \code{fun2} must return a list with at least one element carrying the goodness of fit; the name of this element may be specified through the \code{GoFname} argument, by default "GoF" is assumed.
#'
...
pbcm.du <- function(fun1,
  ...
  for (rep in 1:reps) {
    ...
    data_on_fun1 <- tryCatch({
      do.call(what=genfun1, args=genargs1)
    }, error=function(cond) {
    ...
    data_on_fun2 <- tryCatch({
      do.call(what=genfun2, args=genargs2)
    }, error=function(cond) {
