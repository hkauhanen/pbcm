...
empirical.GoF <- function(data,
  ...
  fun1_to_data <- tryCatch({
    ...
    if (verbose) warning("Fitting fun1 to data failed")
    return(NULL)
  })
  ...
  fun2_to_data <- tryCatch({
    ...
    if (verbose) warning("Fitting fun2 to data failed")
    return(NULL)
  })
